...
function [fidexmult,Xout] = FASTEST_IF(Sig,win_length, num, delta,L,thr,Thr,step,FFT_len)
...
%I=quadtfd(Sig,length(Sig)-1,1,'specx',35,'hamm',256);
    ...
    if sum(abs(s2(length(Sig)/2-LL-1:length(Sig)/2+LL-1).^2))<e_max*thr
        ...
        s11=zeros(size(s2));
        s11(length(Sig)/2-LL:length(Sig)/2+LL)=s2(length(Sig)/2-LL:length(Sig)/2+LL);
        s2(length(Sig)/2-LL:length(Sig)/2+LL)=0;
        s2=ifft(ifftshift(s2)).*conj(s_dechirp);
                s11=ifft(ifftshift(s11)).*conj(s_dechirp);

        [s11,~,~] = ICCD_sparse(Sig,1,IF,3,1,1:length(Sig));
        fidexmult(iii,:) = IF;
        Xout(iii,:)=s11;
